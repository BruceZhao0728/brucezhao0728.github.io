...
最简单的机器学习模型就是**最近邻分类器**。它的基本思想是：对于一张新的图片，我们找到训练集中与它最相似的图片，然后将这张图片的标签作为新图片的预测标签。
...
> 
> **曼哈顿距离**：
> $$ \text{dist}_{\text{M}}(X, Y) = \sum_{h, w, c} |X_{h,w,c} - Y_{h,w,c}| $$
> **欧几里得距离**：
...
既然如此，那我们不妨引入一个改进版本的最近邻分类器，叫做**K近邻分类器**（K-Nearest Neighbors Classifier）。它的基本思想是：对于一张新的图片，我们找到训练集中与它最相似的K张图片，然后根据这K张图片的标签来进行投票，最终将得票最多的标签作为新图片的预测标签。
