...
新的问题出现了：怎么定义单个样本的损失函数呢？我们前面的线性分类器最后的输出是一个得分向量，每一类标签的得分范围是不确定的，我们只有比较他们的相对大小才能知道哪个类别的得分更高，所以我们需要一个损失函数来衡量模型的预测结果与真实标签之间的差距。

一个想法是：我们可以直接比较模型预测的得分与真实标签的得分之间的差距。比如说，我们可以定义一个叫做**多类SVM损失函数**（Multiclass SVM Loss，又称合页损失函数 Hinge Loss）的单个样本损失函数。
...
> [!Warning] 为什么间隔是1呢？
> 在多类SVM损失函数中，我们使用了一个固定的间隔值1来衡量正确分类的得分与错误分类的得分之间的差距。这个阈值是一个超参数，可以根据具体的任务和数据集进行调整。选择一个合适的间隔值可以帮助模型更好地学习到区分不同类别的特征，从而提高分类性能。一般来说，较大的间隔值可以使模型更具有鲁棒性，但也可能导致模型过于保守；较小的间隔值则可能使模型更具有攻击性，但也可能导致模型过拟合。因此，在实际应用中，我们需要通过实验来选择一个合适的间隔值，以获得最佳的分类性能。


另一个想法是：我们可不可以把得分高低转换成概率的形式呢？也就是说，我们希望模型的输出不仅仅是一个得分向量，而是一个概率分布，表示每个类别的预测概率。这就引入了一个很重要的概念：**Softmax函数**。Softmax函数可以将一个实数向量转换成一个概率分布，使得每个类别的预测概率都在0和1之间，并且所有类别的预测概率加起来等于1。
...
我们可以使用交叉熵损失函数（Cross-Entropy Loss）来衡量这个差距。
...
今天我们介绍了线性分类器 $f(x) = Wx + b$ 的基本原理，并且从代数、视觉、几何三个角度理解了它的工作机制：
- 代数角度：权重矩阵 $W$ 是一个变换矩阵，将输入向量 $x$ 映射到一个新的空间中，每个类别对应一个维度。
- 视觉角度：权重矩阵的每一行可以看作是一个类别的“模板”，当输入图片与这个模板越相似时，得分就越高，分类结果就越倾向于这个类别。
- 几何角度：线性分类器可以看作是在输入空间中划分不同类别的超平面，每个类别对应一个超平面，当输入向量落在某个类别的区域内时，线性分类器就会将其分类为该类别。

我们还讨论了什么是一个好的线性分类器，并引入了损失函数来衡量模型的性能：
